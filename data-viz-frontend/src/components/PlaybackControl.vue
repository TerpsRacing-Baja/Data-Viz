...
function getCurrentTimeSeconds(): number {
 if (!data_csv.value || data_csv.value.length === 0) return 0;
 const index = Math.min(time.value, data_csv.value.length - 1);
 const row = data_csv.value[index];
 const key = 'Interval|"ms"|0|0|1';
 const ms = Number(row[key]);
 return isNaN(ms) ? 0 : ms / 1000;
}
</script>
<template>
 <div id="playback">
   <label>Scrub a dub dub: {{getCurrentTimeSeconds()}} seconds</label>
   <input
     v-model="time"
     type="range"
     min="1"
